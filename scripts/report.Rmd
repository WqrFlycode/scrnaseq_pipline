...
样本**`r info$data_name`**共包含**`r info$raw_dim[1]`**个基因和**`r info$raw_dim[2]`**个细胞。
...
经过质量控制，
剔除了 **`r info$raw_dim[1]-info$filter_dim[1]`** 个基因，
剩余 **`r info$filter_dim[1]`** 个基因，
剔除了 **`r info$raw_dim[2]-info$filter_dim[2]`** 个细胞，
剩余 **`r info$filter_dim[2]`** 个细胞。
...
```
从图中可以看出，聚类后所有细胞被分成**`r length(info$celltypes)`**类。
...
## 每个细胞信号图
下图分别展示了每个细胞与其余细胞的细胞互作，圆圈的大小表示细胞数量，线的粗细表示细胞信号的数量。
```{r echo=FALSE}
filename <- "each_cell_interaction"
if(length(grep(filename, files)) >= 1){
  knitr::include_graphics(paste0(dir_C, files)[grep(filename, files)])
}
```
